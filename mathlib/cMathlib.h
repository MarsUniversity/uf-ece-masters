...
class cBaseMath {
 public:
  ...
  std::string name;     //!< name of cMathlib object
...
class cVector : public cBaseMath {
 ...
 public:
  cVector(int,const std::string&,ml_data* =NULL);
  cVector(int,ml_data* =NULL);
  cVector(void){size=0;p=NULL;}
...
cMatrix& skew(const ml_data*,int);  // skew matrix
inline cMatrix& skew(const ml_data* d){ return skew(d, 3); }  // skew matrix
cMatrix& eye(int);            // identity matrix
cMatrix& zeros(int,int);      // a matrix of zeros
cMatrix& ones(int,int);       // a matrix of ones
cMatrix& inv(const cMatrix&); // a matrix inverse
...
class cMatrix : public cBaseMath {
 ...
 public:
  cMatrix(int,int,const std::string&,const ml_data* =NULL);	// constructor
  cMatrix(int,int,const ml_data* =NULL);	// constructor
  cMatrix(void){r=c=0;p=NULL;}
  ~cMatrix();  // destructor
